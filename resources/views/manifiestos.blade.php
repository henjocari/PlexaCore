                        ...
                        <div class="card-body p-3">
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover text-nowrap w-100" id="tablaManifiestos">
                                    ...
                                    {{-- ✅ Solo @foreach: si no hay registros el tbody queda vacío
                                         y DataTables muestra su propio mensaje (evita el warning de column count) --}}
                                    <tbody>
                                    @foreach($manifiestos as $manifiesto)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <span class="badge badge-success px-2 py-1">{{ $manifiesto->tipoOperacion ?? 'N/A' }}</span>
                                            </td>
                                            ...
                                            <td class="text-right font-weight-bold text-dark">$ {{ number_format($manifiesto->fleteNeto ?? 0, 0, ',', '.') }}</td>
                                            <td class="text-right text-info">$ {{ number_format($manifiesto->anticipo ?? 0, 0, ',', '.') }}</td>
                                            <td class="text-right">$ {{ number_format($manifiesto->reteIca ?? 0, 0, ',', '.') }}</td>
                                            <td class="text-right">$ {{ number_format($manifiesto->reteFuente ?? 0, 0, ',', '.') }}</td>
                                            <td class="text-right">$ {{ number_format($manifiesto->fopat ?? (($manifiesto->fleteNeto ?? 0) * 0.001), 0, ',', '.') }}</td>
                                            <td class="text-right font-weight-bold text-danger">$ {{ number_format($manifiesto->saldoPagar ?? 0, 0, ',', '.') }}</td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
