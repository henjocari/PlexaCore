...
<head>
    ...
    <style>
        ...
        table.pdf-header { width: 100%; border-collapse: collapse; margin-bottom: 14px; }
        .pdf-header td { border: 1px solid #000; padding: 6px; text-align: center; vertical-align: middle; }
        .pdf-title { font-weight: bold; font-size: 11px; text-transform: uppercase; }
        .pdf-meta { font-size: 9px; line-height: 1.3; }
        .pdf-text { text-align: justify; font-size: 11px; margin-bottom: 10px; line-height: 1.5; }
        ol.pdf-list { text-align: justify; font-size: 11px; margin: 0 0 10px 0; padding-left: 20px; }
        ol.pdf-list li { margin-bottom: 6px; }
        ul.pdf-arrows { list-style: none; text-align: justify; font-size: 11px; margin: 0 0 10px 0; padding-left: 5px; }
        ul.pdf-arrows li { margin-bottom: 6px; }
        .autorizacion-line { margin: 6px 0 10px 0; font-size: 11px; }
        .checkbox-marcado { display: inline-block; width: 14px; height: 14px; border: 1.5px solid #000; background: #fff; color: #000; text-align: center; line-height: 12px; font-size: 10px; font-weight: bold; vertical-align: middle; margin-right: 4px; }
        .checkbox-text { font-weight: bold; margin-right: 16px; }
        .field-line { margin-bottom: 8px; font-size: 11px; }
        .field-value { border-bottom: 1px solid #000; display: inline-block; min-width: 250px; font-weight: bold; }
        .signature-area { margin-top: 14px; page-break-inside: avoid; }
        .firma-img { max-height: 70px; width: auto; display: block; margin-bottom: 2px; }
        .signature-line { border-top: 1px solid #000; width: 220px; margin: 2px 0 4px 0; }
...
<body>
    ...
    <div class="page">
        <table class="pdf-header">
            <tr>
                <td rowspan="3" style="width: 20%;">
                    ...
                    @else
                        <span style="font-size:9px; color:#999;">Logo no disponible</span>
                    @endif
                </td>
        ...
        <div class="autorizacion-line">
            <span class="checkbox-marcado">@if($firma->autorizo_personales === 'SI')✓@endif</span><span class="checkbox-text">SI</span>
            <span class="checkbox-marcado">@if($firma->autorizo_personales === 'NO')✓@endif</span><span class="checkbox-text">NO</span>
        </div>
    </div>
    ...
    <div class="page">
        ...
        <div class="pdf-text" style="font-weight: bold; margin-bottom: 2px;">Autorizo:</div>
        <div class="autorizacion-line">
            <span class="checkbox-marcado">@if($firma->autorizo_sensibles === 'SI')✓@endif</span><span class="checkbox-text">SI</span>
            <span class="checkbox-marcado">@if($firma->autorizo_sensibles === 'NO')✓@endif</span><span class="checkbox-text">NO</span>
        </div>
    ...
    <div class="page">
        ...
        <div class="signature-area">
            <div class="field-line"><strong>NOMBRE COMPLETO DEL TITULAR:</strong> <span class="field-value">{{ $firma->nombre }}</span></div>
            <div class="field-line"><strong>DOCUMENTO DE IDENTIDAD NO.:</strong> <span class="field-value">{{ $firma->cedula }}</span></div>
            <div class="field-line"><strong>FECHA DE AUTORIZACIÓN:</strong> <span class="field-value">{{ $firma->fecha_autorizacion ? \Carbon\Carbon::parse($firma->fecha_autorizacion)->format('d/m/Y') : '' }}</span></div>

            <div style="margin-top: 12px;">
                <strong>FIRMA TITULAR:</strong><br>
                @if($firma->firma && strpos($firma->firma, 'data:image') === 0)
                    <img src="{{ $firma->firma }}" class="firma-img" alt="Firma">
                @else
                    <span style="font-size:9px; color:#999;">Firma no disponible</span>
                @endif
